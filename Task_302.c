...
char snakecollide(Rect *snake, int size, Rect food, Rect poison_food, bool poison_active) 
{
    ...
    if (poison_active && isInRect(snake[0], poison_food)) 
    {
        return 'p';  
    }
...
int main(int ac, char *av[]) 
{
    ...
    poison_food.width = poison_food.height = FOOD_W_H;
    ...
    while (running) 
    {
        ...
        if (poison_active) 
        {
            ...
            regularfood(ren, poison_food.x + FOOD_W_H / 2, poison_food.y + FOOD_W_H / 2, FOOD_W_H / 2);
            if (SDL_GetTicks() - poison_start_time > 4000) 
            {
                poison_active = false;
            }
        }
        ...
        char coll = snakecollide(snake, len, food, poison_food, poison_active);
        ...
        if (coll == 'f') 
        {
            ...
            if (food_counter == 4) 
            {
                poison_food.x = 20 * (rand() % 25);
                poison_food.y = 20 * (rand() % 25);
                poison_active = true;
